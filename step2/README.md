...
- [ ] 번들러 조사
  ...
  - [ ] 번들을 하는 이유는?
    ...
    - 보안을 위해 사용자가 직접 코드를 보기 어렵도록 코드를 난독화할 수 있다.
    - 다양한 브라우저 환경에서 문제가 없도록 가공/변환해준다.

  - 모듈?
    - 소프트웨어의 크기가 방대해지면 기능별로 파일을 분리한다. 이 각각의 파일들을 모듈이라고 한다.
    - `export`와 `import`를 사용하면 필요할 때 모듈을 불러와 그 모듈의 함수를 호출할 수 있다.
  ...
  - [ ] 모듈 시스템에 대해 알아보기
    ...
    - [ ] 브라우저 모듈
     ...
     - 모듈에는 `.mjs` 확장자를 사용할 수 있다. 안 써도 되지만 코드를 안 봐도 모듈인지 알 수 있다.

  - [ ] 번들러 종류 [모듈 시스템 / 번들러 비교](https://wormwlrm.github.io/2020/08/12/History-of-JavaScript-Modules-and-Bundlers.html)
    - [ ] parcel
     - 빌드 실행과 번들 속도가 빠르다.
     - 특별한 설정이 없어도 된다. (Webpack은 설정이 까다롭다고 함.)
     - HMR 지원
