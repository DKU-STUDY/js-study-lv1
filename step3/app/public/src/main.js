...
function registerEvent() {
  document.querySelectorAll('.check').forEach(assignToggleEvent);
  document.querySelectorAll('.toggle').forEach(assignToggleEvent);
  document.querySelectorAll('.edit').forEach(assignEditEvent);
  document.querySelectorAll('.remove').forEach(assignRemoveEvent);
};

// Register event to button
function assignToggleEvent(item) {
  item.addEventListener('click', toggleItem);
}
function assignEditEvent(item) {
  item.addEventListener('click', editItem);
}
function assignRemoveEvent(item) {
...
function editItem(event) {
  ...
  $parent.innerHTML = `<form id=editForm action="">
  <fieldset>
